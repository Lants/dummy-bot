...
class Onuw(commands.Cog):
    ...
    class OnuwGame(object):
        def __init__(self, IDList, origin_channel):
        ...
        async def sortRoles(self):
            ...
            if n == 3:
                ...
                self.__roleList = [1, 2, 3, 4, 5, 7, 10] # Werewolf Minion Seer Robber Troublemaker Drunk Insomniac
            elif n == 5:
                self.__roleList =  [1, 2, 3, 4, 5, 7, 10, 0]
            elif n == 6:
                self.__roleList = [1, 6, 3, 4, 5, 7, 9, 10, 9] # Replaced minion with tanner
            elif n == 7:
                self.__roleList = [1, 2, 3, 4, 5, 7, 8, 10, 0, 0] # Werewolf Minion Seer Robber Troublemaker Hunter Insomniac Villager Villager
            elif n == 8:
        ...
        async def endGame(self, ctx):
            print("ONUW GAME ENDED")
            results = "```End Roles:\n"
            for player in await self.getPlayerCards():
                results += str(player)
                results += "\n"
            results += "```"
            await self.__origin_channel.send(results)
        ...
        async def startGame(self, ctx):
            ...
            def checkReactionStartGame(reaction, user):
                return (
                    (
                        reaction.emoji == "\U0001F43A" and
                        reaction.message.channel.id == self.__IDList[NARRATOR] and
                        reaction.count > len(self.__playerList)
                    )
                    or reaction.emoji == "🥑" and user.id is ctx.guild.owner.id
                )
            ...
            if len(self.__cardArray[SEER]) > 0:
                ...
                # Default to checking the first two cards
                card_a = 0
                card_b = 1
                is_checking_middle = True # Seer chooses to view middle deck?

                try: # If reaction was received
                    if reac.emoji == "1️⃣":
                        card_a = 1
                        card_b = 2
                    elif reac.emoji == "2️⃣":
                        ...
                        card_a = 0
                        card_b = 1
                    else: # If reaction corresponds to a player
                        is_checking_middle = False
                        for player in self.__playerList:
                            if reac.message.content == player.getUser().name:
                                ...
                                for seer in self.__cardArray[SEER]:
                                    print(seer)
                                    await seer.getUser().send("%s's card is: %s" % (player.getUser().name, self.__RoleSwitcher.get(player.getRole())))
                except asyncio.TimeoutError as e: # If timeout
                    print("Seer reaction error: %s. Showing cards 0 and 1." % e)
                
                finally:
                    if is_checking_middle:
                        await secret.send("Cards %d & %d are: %s, %s" % (card_a, card_b, self.__RoleSwitcher.get(self.__tableCards[card_a]), self.__RoleSwitcher.get(self.__tableCards[card_b])))
                        for seer in self.__cardArray[SEER]:
                            await seer.getUser().send("Cards %d & %d are: %s, %s" % (card_a, card_b, self.__RoleSwitcher.get(self.__tableCards[card_a]), self.__RoleSwitcher.get(self.__tableCards[card_b])))
            ...
            if len(self.__cardArray[DRUNK]) > 0:
                ...
                try:
                    ...
                    print("Drunk error, switching with card 1: %s" % e)
                
                finally:
                    for drunkard in self.__cardArray[DRUNK]:
                        await self.__cardArray[DRUNK][0].setRole(tempCard)
                        await drunkard.getUser().send("Drunk: You swapped with card %d" % cardNo)
            ...
            if INSOMNIAC in self.__roleList:
                ...
                for player in self.__cardArray[INSOMNIAC]:
                    await player.getUser().send("Your post-insomniac role is: %s" % self.__RoleSwitcher.get(player.getRole()))
                await asyncio.sleep(5)
    ...
    @commands.command(name = "ONUW", description = "Play One Night Ultimate Werewolf!\nUsage: !ONUW <user1> <user2> <user3>...\nwhere <user#> is a player")
    @commands.check_any(commands.has_permissions(administrator = True), commands.is_owner())
    async def onuw(self, ctx, *args):
        """Play One Night Ultimate Werewolf!"""
        channel = ctx.channel
        await channel.send("Welcome to One Night Ultimate Werewolf! It may take a second to set up, but please make your way over to the 'onuw-narrator' channel :)")

        mentionList = ctx.message.mentions
        
        IDList = await self.onuwChannel(ctx, "x")
        onuwInstance = self.OnuwGame(IDList, channel)
        ...
        try:
            ...
            await onuwInstance.endGame(ctx)
            await self.onuwChannel(ctx, 'd')
    ...
    @commands.check_any(commands.has_permissions(administrator = True), commands.is_owner())
    async def onuwRoles(self, ctx, arg):
    ...
    @commands.check_any(commands.has_permissions(administrator = True), commands.is_owner())
    async def onuwChannel(self, ctx, arg):
        print("Running ONUWChannel")
        for chan in ctx.guild.channels:
            if "onuw-secret" == chan.name or "onuw-narrator" == chan.name or "onuw-dead" == chan.name:
                await chan.delete()

        onuwRoleIDList = await self.onuwRoles(ctx, arg)
        print(onuwRoleIDList)

        if arg != "d":
            overwritesSecret = {
                ctx.guild.default_role: PermissionOverwrite(read_messages = False),
                ctx.guild.get_role(onuwRoleIDList[AWAKE]): PermissionOverwrite(read_messages = True),
                ctx.guild.get_role(onuwRoleIDList[DEAD]): PermissionOverwrite(send_messages = False)
            }

            overwritesNarrator = {
                ctx.guild.default_role: PermissionOverwrite(read_messages = False),
                ctx.guild.get_role(onuwRoleIDList[ALIVE]): PermissionOverwrite(read_messages = True),
                ctx.guild.get_role(onuwRoleIDList[DEAD]): PermissionOverwrite(send_messages = False),
                ctx.guild.get_role(onuwRoleIDList[DEAD]): PermissionOverwrite(add_reactions = False),
                ctx.guild.get_role(onuwRoleIDList[DEAD]): PermissionOverwrite(read_messages = True),
            }
