...
class Chat(commands.Cog):
    ...
    # !geno
    # geno's secret santa gift
    @commands.command(name = "geno")
    async def geno(self, ctx, password):
        """Geno's secret santa gift! Enter !geno <password> to unlock :)"""
        if password != None and isinstance(password, str):
            if password == "GiveMeAMinute":
                await ctx.send("🎄🎅 Merry Christmas Geno! 🎅🎄\nHere's your ⛏️ Minecraft code:\nFRJHF-9P37H-D9VH7-X3TDY-49QHZ")
            else:
                await ctx.send("That's not a valid password!")
        else:
            await ctx.send("That's not a valid password!")
    ...
    @geno.error
    async def geno_error(self, ctx, error):
        await ctx.send("That's not a valid password!")
